...
(function(){
  ...
  const target = new Date(2026, 4, 30, 12, 0, 0); // May is month 4 (0-based)
  ...
  function update(){
    ...
    let diff = target - now;
    if (diff <= 0){
      cdRoot.innerHTML = '<p class="lead">Il matrimonio è già passato (30 maggio 2026).</p>';
      return;
    }
...
function toggleMusic() {
  if (!playing) {
    ...
    audio.pause();
    playing = false;
    document.getElementById('music-btn').innerHTML = '🎶 Play musica';
  }
}
